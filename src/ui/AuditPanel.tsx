 ...
 *
 * `bare` drops the card chrome and the scroll cap for use inside a drawer,
 * which supplies both.
 */
export function AuditPanel({ bare = false }: { bare?: boolean } = {}) {
  const { audit } = useStore()
  const emitted = audit.some((entry) => entry.fields && entry.fields.length > 0)

  const explainer = emitted ? (
    <div
      className={
        bare
          ? 'mb-3 text-[12.5px] leading-relaxed text-muted'
          : 'border-b border-line bg-muted-bg px-5 py-2.5 text-[12px] leading-relaxed text-muted'
      }
    >
  ...
  const body: ReactNode =
    audit.length === 0 ? (
      ...
      <div className={bare ? '' : 'max-h-72 overflow-y-auto'}>
        {audit.map((entry, i) => (
          <div
            key={`${entry.at}-${i}`}
          ...
          >
            ...
            <div className="min-w-0">
              <span className="block truncate text-ink" title={entry.detail ?? entry.action}>
                {entry.action}
                {entry.outcome !== 'ok' && <span className="text-danger"> ({entry.outcome})</span>}
              </span>
